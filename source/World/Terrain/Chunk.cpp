...
Vertex *Chunk::s_vertices = nullptr;
uint *Chunk::s_indices = new uint[6*12*16*16*3];

// BLOCK QUADS
Chunk::BlockQuad::BlockQuad() :
	block(BLOCK_EMPTY)
{
}

Chunk::BlockQuad::BlockQuad(BlockID block, const float x0, const float y0, const float x1, const float y1, const float u0, const float v0, const float u1, const float v1) :
	block(block),
	x0(x0),
	y0(y0),
	x1(x1),
...
void Chunk::generateVertexBuffer(ChunkLoader *chunkLoader, TerrainLayer z)
{
	// Get adjacent chunks
