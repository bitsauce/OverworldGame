...
void ItemData::init(OverworldGame *game)
{
	...
	(s_data[ITEM_PICKAXE_IRON] = new Pickaxe(game))->set(ITEM_PICKAXE_IRON, "Iron Pickaxe", "An iron pickaxe", "Sprites/ItemIcons/IronPickaxe", 1);
	(s_data[ITEM_AXE_IRON] = new Axe(game))->set(ITEM_AXE_IRON, "Iron Axe", "An iron axe", "Sprites/ItemIcons/IronAxe", 1);

	(s_data[ITEM_TORCH] = new BlockEntityItem(game, BLOCK_ENTITY_TORCH))->set(ITEM_TORCH, "Torch", "Useful light source", "Sprites/ItemIcons/Torch", 255);
	...
	(s_data[ITEM_BOW_WOODEN] = new Bow(game))->set(ITEM_BOW_WOODEN, "Bow", "", "Sprites/ItemIcons/WoodenBow", 1);
	(s_data[ITEM_ARROW] = new Ammo(game))->set(ITEM_ARROW, "Arrow", "Needs a bow to fire", "Sprites/ItemIcons/Arrow", 255);
}