...
void InGameDebug::debugFunction(InputEvent *_e)
{
	...
	switch(e->getKeycode())
	{
		...
		case SAUCE_KEY_F11:
		{
			...
			for(int y = area.y0 * CHUNK_BLOCKS; y <= area.y1 * CHUNK_BLOCKS; ++y)
			{
				...
				{
					m_world->getTerrain()->createBlockEntityAt(x, y, BlockEntityData::GetByName("torch"), true);
				}
			}
		}
		break;
...
void InGameDebug::onTick(TickEvent *e)
{
	if(!m_enabled) return;
...
void InGameDebug::onDraw(DrawEvent *e)
{
	if(!m_enabled) return;
...
void InGameDebug::onKeyEvent(KeyEvent *e)
{
	switch(m_debugMode)
	{
		case DEBUG_MODE_LIGHT_PAINTER:
...
void InGameDebug::prevBlock(InputEvent *e)
{
	if(m_enabled && e->getType() == KeyEvent::DOWN)
	{
...
void InGameDebug::addVariable(const string &name, const string &value)
{
